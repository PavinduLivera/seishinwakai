"use client";
import { useRef, useState, FormEvent } from "react"; // Added FormEvent type
import emailjs from "@emailjs/browser";
import { FaClock, FaEnvelope, FaPaperPlane, FaPhoneAlt, FaWhatsapp } from "react-icons/fa";
import PageHeader from "../components/PageHeader";

export default function ContactForm() {
  // FIX 1: Initialized with null and added HTMLFormElement type
  const form = useRef<HTMLFormElement>(null); 
  const [loading, setLoading] = useState(false);

  // FIX 2: Added Type for the event 'e'
  const sendEmail = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    
    // Safety check for TypeScript: ensures form.current is not null
    ...
    .then((result) => {
        ...
        // Use optional chaining or a null check here
        form.current?.reset(); 
        setLoading(false);
    }, (error) => {
  ...
  return (
    <main className="bg-white min-h-screen">
      {/* ... keeping your existing JSX the same ... */}
      <PageHeader 
        title="Get In" 
        highlight="Touch" 
